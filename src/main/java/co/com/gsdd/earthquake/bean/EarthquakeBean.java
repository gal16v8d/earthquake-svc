...
public class EarthquakeBean implements Serializable {
    ...
    public String consultarSismos() {
        ...
        if (isByDateRange()) {
            ...
            if (calcDays > 30 || calcDays < 0) {
                log.info("Can not perform search if days are not in range [0,30], current value: {}", calcDays);
                features.setRowCount(0);
                return INDEX_FACES;
            }
        }
    ...
    public void showDetail(EarthquakeProps props, String rowKey, EarthquakeCoordinates geometry) {
        ...
        getSelectedEarthquake().setGoogleMaps(String.format(GOOGLE_MAPS_QUERY, coordinates, coordinates));
